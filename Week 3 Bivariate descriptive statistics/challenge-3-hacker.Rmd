...
Write the code to answer each question in the shaded code chunks. Below the code chunks, write an interpretation of your results.
...
(3) Use appropriate numbers and a graph to describe the relationship between age (age) and years smoking (yearssmoking). Interpret your results.
...
(4) Take a subset of the data to delete the values of age that do not make sense and revise your graph and numbers from question 3. Explain what changed in your graph and numbers. Hint: to remove values of age that do not make sense, use age < 100 or something similar in the subset command
...
(5) Use appropriate numbers and graphs to describe the relationship between weight status (VBMI4CAT) and sex (sex). Interpret the results.
...
(6) Use subset to remove people with weight status of "Tall" and check your results.
...
(7) Use subset to remove blank values. Re-do the analysis from number (5). Compare the results with your results from number (5). Hint: to remove blank values, put nothing between the quote marks in the subset command
...
(8) Create a stacked bar graph and a grouped bar graph to visualize the relationship from question 7. Explain what each graph is showing. Compare the stacked and grouped graphs and make a case for which one is preferable for understanding the data. Tip: see http://docs.ggplot2.org/current/geom_bar.html and http://rstudio-pubs-static.s3.amazonaws.com/3256_bb10db1440724dac8fa40da5e658ada5.html 
...
(9) Look up the droplevels command and demonstrate what it does by using droplevels on the VBMI4CAT variable and showing the levels before and after.

```{r question9}
# using droplevels to clean VBMI4CAT

```

Interpretation for 9: 
