...
You’ve been asked to do quality control for a local factory. Consider the packages of M&Ms distributed across the entire class to be the whole population of M&Ms produced in this factory during a certain time frame.

# Your tasks

(1)	Count how many blue M&Ms are in each of your M&M packages. IMPORTANT: Count each package separately, do not mix package contents together before counting. Write these numbers on the board.
...
Since all the bags in the class are the populatoin, this value is the population mean. Population values are often written using Greek characters. Write the population mean with the Greek character for mean (µ pronounced mu).

µ = 

(3)	Use R to randomly select a sample of the number of blue m&ms from 10 packages from the mm vector and compute the mean and standard deviation. 

```{r}
# take a sample of 10 values from the mm vector
...
Take another sample and find the mean and standard deviation:
...
(6)	Count how many of the sample means are within 1 standard deviation of the sampling distribution mean. Count how many are within 2 standard deviations. What percentage are within 1 and 2 standard deviations of the sampling distribution mean?
