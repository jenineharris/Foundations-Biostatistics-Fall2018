...
There are several questions about heroin use in the NHANES Drug Use questionnaire:
...
```{r}
# descriptives for drug use
drug.use <- cbind(table(nhanes2013$ever.heroin),
                    prop.table(table(nhanes2013$ever.heroin)))
drug.use
...
```{r}
# move row labels to a variable and recode
# No1 and Yes1 to No and Yes
...
kable(drug.use.all, "html", caption = "Drug use among NHANES 2013-2014 participants",
  ...
  group_rows("Age of first use (median, IQR)", 5, 6)
```

Ok! There are lots of things you can do with `kable` to style and save your table. Use the Google to see what is out there.

## A new graphing option (and some code to make maps)

In past weeks we have used:

* bar graphs for one or two categorical variables 
* mosaic plots for two categorical variables 
* histograms and density plots for continuous variables 
* box plots for categorical and continuous variables together
* petal plots for categorical and continuous variables together 
* scatterplots for two continuous variables together 

This section offers one more option (and some code for maps): 

* waffle plot for categorical variables (instead of pie graph to display parts of a whole)
...
ggplot(pieExample, aes(x="", y=percentPie, fill=colorPie)) +blank_theme+
...
The `ggplot2` package has a geom for mapping. It can take global, state, or county-level data. We will use incarceration data in from wikipedia.

The map will need two things: (1) a variable called `region` with state names in all-lower case letters, (2) a numeric or factor variable of something you want to map.
...
# basic stuff for all state maps does not need to change
ggplot() + 
...
Pick the standard or hacker challenge RMD file from GitHub. Write the code and text needed to answer the questions. Submit via Blackboard before the next class meeting.
