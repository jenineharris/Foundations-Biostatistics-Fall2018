...
chi.animals <- CrossTable(x = animal.injuries$animal.type,   # enter one of the variables as x
                          y = animal.injuries$injured,       # enter the other variable as y
                          chisq=TRUE)                        # include the chi-squared in the output
chi-animals
```
...
chi.animals.simp <- CrossTable(x = animal.injuries$animal.type,   # enter one of the variables as x
                               y = animal.injuries$injured,       # enter the other variable as y
                               chisq = TRUE,                      # include the chi-squared 
...
Choose the appropriate statement and fill in the value of chi-squared and p:
...
We can visualize this chi-squared statistic on the distribution where the orange dotted lines shows us the cutoff for extreme values where the area under the curve to the right contains 5% of observations. The blue line is our chi-squared statistic. Run the code chunk to see the chi-squared distribution with the value computed.

```{r echo=FALSE}
# visualization of chi-squared distribution with chi-squared
# from animal injury study
curve(dchisq(x, df = chi.animals.simp$df), main = "Chi-squared dist",
          from=0,to=40, xlab="chi-squared", ylab="probability under null")
abline(v = qchisq(.95, df = chi.animals.simp$df), col="orange", lty="dashed")
abline(v = chi.animals.simp$chisq, col = "blue")
```

# Learning more about the relationship OPTION 1

The chi-squared statistic determines whether or not there is a statistically significant relationship between two categorical variables. To know what the relationship is, we can use the standardized residuals (like z-scores) to determine which of the frequencies in the CrossTable are significantly greater (or smaller) than we would expect to see just by chance. 

The standardized residual subtracts the expected value in a cell from the observed value in a cell and divides by the square root of the expected value (think about standard error!):

\[
standardizedresidual =\frac{observed - expected}{\sqrt{expected}}
\]
...
chi.animals.simp <- CrossTable(x = animal.injuries$animal.type,   # enter one of the variables as x
                               y = animal.injuries$injured,       # enter the other variable as y
                               chisq = TRUE,                      # include the chi-squared 
                               sresid = TRUE,                     # include the standaridized resid
                               prop.t = FALSE, prop.c = FALSE,    # remove percentages
                               prop.chisq = FALSE)                # remove percentages
chi.animals.simp
```
...
_Is there a relationship between sex and smoking status?_

Include at least: 

* The appropriate descriptive statistics and a graph examining the relationship before analyses 
...
```{r}
# descriptive statistics and a graph examining relationship 
